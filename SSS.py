...
def behind_the_scene_create_account():
    ...
    if len(y) == 4:
        ...
        if z in records.keys():
            ...
            f.close()
            fp.close()
            answer = tkinter.messagebox.askyesno("INFO", "Account created successfully. Continue?")
...
def online():
    def amount_transferred():
        ...
        if temp_data[z][2] >= int(online_entry_2.get()) and m in records.keys():
            ...
            tkinter.messagebox.showerror("ERROR", " INVALID ACCOUNT NAME")

    online_frame = Frame(display_frame, width=900, height=400)
    online_frame.place(x=0, y=0)
    online_frame.tkraise()
    online_label = Label(online_frame, text="ENTER THE ACCOUNT YOU WANT TO TRANSFER INTO")
    online_label.place(x=325, y=150)
    online_entry = Entry(online_frame)
    online_entry.place(x=400, y=190)
    online_label_2 = Label(online_frame, text="ENTER THE AMOUNT")
    online_label_2.place(x=390, y=260)
...
def deposit():
    ...
    deposit_label = Label(deposit_frame, text="ENTER THE AMOUNT YOU WANT TO DEPOSIT")
...
def portal():
    ...
    dasboard_button_2.place(x=265, y=50)
    dasboard_button_3 = Button(dasboard_frame, text='WITHDRAW', command=withdraw)
    dasboard_button_3.place(x=375, y=50)
    dasboard_button_5 = Button(dasboard_frame, text='ONLINE TRANSFER', command=online)
    dasboard_button_5.place(x=500, y=50)
    dasboard_button_4 = Button(dasboard_frame, text='LOGOUT', command=logout)
    dasboard_button_4.place(x=675, y=50)
...
root.title("Sargeant ATM System")
img1=PhotoImage(file='portal_logo.png')
img2=PhotoImage(file='main_logo.png')
main_window()
root.mainloop()
